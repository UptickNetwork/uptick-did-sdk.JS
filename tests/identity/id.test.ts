...
describe('identity', () => {
  ...
  it('createProfile', async () => {
    ...
    const tags3 = ['senderAddress=0x456'];
    const tags4 = ['app=web'];
    const tags5 = ['senderAddress=0x678'];
    ...
    const profileDid3 = await idWallet.createProfile(did, 12, verifier, {
      tags: tags2
    });
    ...
    await idWallet.createProfile(did, 13, verifier, {
      tags: tags3,
      didDocument
    });
    const profileWithDidDocFromParams = await idWallet.getProfilesByVerifier(verifier, tags3);
    expect(profileWithDidDocFromParams.length).to.be.eq(1);
    expect(profileWithDidDocFromParams[0].did_doc).to.deep.equal(didDocument);

    const keyProvider = new RsaOAEPKeyProvider(new InMemoryPrivateKeyStore());
    const profileDid4 = await idWallet.createProfile(did, 14, verifier, {
      tags: tags4,
      encryptionKeyOps: {
        provider: keyProvider
      }
    });
    const profileWithAutoGeneratedDidDoc = await idWallet.getProfilesByVerifier(verifier, tags4);
    expect(profileWithAutoGeneratedDidDoc.length).to.be.eq(1);
    const [{ did_doc: didDoc }] = profileWithAutoGeneratedDidDoc;
    expect(didDoc?.id).to.equal(profileDid4.string());
    expect(didDoc?.verificationMethod?.length).to.equal(1);
    const [{ publicKeyJwk }] = didDoc?.verificationMethod || [];
    expect(publicKeyJwk?.alg).to.equal(PROTOCOL_CONSTANTS.AcceptJweKEKAlgorithms.RSA_OAEP_256);

    const keyId = await keyProvider.newPrivateKey();
    const profileDid5 = await idWallet.createProfile(did, 15, verifier, {
      tags: tags5,
      encryptionKeyOps: {
        provider: keyProvider,
        alias: keyId.id
    ...
    const profileWithAlias = await idWallet.getProfilesByVerifier(verifier, tags5);
